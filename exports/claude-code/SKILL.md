...
<!-- ⚠️ DISTRIBUTION COPY: This file is a distribution copy of human2skill-meta/SKILL.md.
     The canonical source lives in human2skill-meta/SKILL.md.
     When editing, update the canonical file first, then sync here. -->
...
```bash
human2skill create --root workspace --slug zhang-san --name "张三" \
  --profile colleague --relationship "coworker" --use-case "work perspective advisor" \
  --voice-mode advisor
```
...
```bash
human2skill ingest --root workspace --slug zhang-san --file ./notes/zhangsan-meeting.md
```
...
```bash
human2skill build --root workspace --slug zhang-san
```

（默认使用 `private_evidence/distillation.json`；可通过 `--distillation` 覆盖。）
...
```bash
human2skill export --root workspace --slug zhang-san --host claude-code --variant advisor
```
